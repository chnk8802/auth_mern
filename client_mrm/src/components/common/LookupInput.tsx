...
import { type LookupField as LookupFieldType } from "@/lib/form-generator/types/field-types";

type Props = {
  field: LookupFieldType;
...
export function LookupInput({ field, value, onChange, disabled }: Props) {
  ...
  const filtered = options.filter((item) =>
    item[field.displayField]?.toLowerCase().includes(search.toLowerCase())
  );

  const selectedLabel = options.find((item) => item.id === value)?.[
    field.displayField
  ];
