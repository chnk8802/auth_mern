  ...
  useEffect(() => {
    async function loadOptions() {
      try {
        ...
        const hasSelected = fetchedOptions.some((item: any) => item._id === value);
        if (value && !hasSelected) {
          const selected = await fetchLookupOptions(
            field.module,
            [field.displayField],
            { _id: value }
          );
        ...
        setOptions([{ _id: "", [field.displayField]: "None" }, ...fetchedOptions]);
      } catch (error) {
    ...
    loadOptions();
  }, [field.module, search, value]);

  const selectedLabel = options.find((item) => item._id === value)?.[field.displayField];
