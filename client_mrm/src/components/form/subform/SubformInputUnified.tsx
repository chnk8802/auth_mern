...
type Props = {
  field: { id: string; fields: ModuleField[] };
  disabled?: boolean;
  minRows?: number;
  maxRows?: number;
};
...
export function SubformInputUnified({
  ...
  const value: any[] = api.formData[field.id] ?? [];
  ...
  const removeRow = (index: number) => {
    ...
    api.removeSubformRow(field.id, index);
  };
  ...
  const saveRow = () => {
    if (editingIndex === null) api.addSubformRow(field.id, rowDraft);
    else api.updateSubformRow(field.id, editingIndex, rowDraft);
    setModalOpen(false);
  };
          ...
          <TableBody>
            ...
            {value.map((row, index) => (
              <TableRow key={index}>
                ...
                {field.fields.map((f: ModuleField) => {
                  const childState = api.getSubformFieldState(field.id, row, f.id);
                  const effectiveValue =
                    childState?.valueOverride ?? row[f.id];
                  ...
                  return (
                    <TableCell key={f.id}>
                      <FieldRenderer
                        ...
                        onChange={(val) =>
                          api.updateSubformRow(field.id, index, { [f.id]: val })
                        }
                        showLabel={false}
                        disabled={childState?.disabled}
                        readOnly={childState?.readOnly}
                      />
                    </TableCell>
                  );
                })}
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            {field.fields.map((f: ModuleField) => {
              const childState = api.getSubformFieldState(field.id, rowDraft, f.id);
              const effectiveValue =
                childState?.valueOverride ?? rowDraft?.[f.id];
              ...
              return (
                <FieldRenderer
                  ...
                  disabled={childState?.disabled || disabled}
                  readOnly={childState?.readOnly}
                  showLabel={true}
                />
              );
